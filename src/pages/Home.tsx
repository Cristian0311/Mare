    ...
    <div className="space-y-12 md:space-y-20 pb-12">
      ...
      <section className="bg-gray-50 border border-gray-100 rounded-3xl p-6 md:p-10 text-center flex flex-col items-center">
        ...
        {config.delivery?.pickupLocations?.[0]?.address && (
          <div className="w-full max-w-3xl relative overflow-hidden bg-white rounded-[2.5rem] border border-gray-100 p-8 md:p-12 shadow-sm mt-4 text-left">
            {/* Background Map Decoration */}
            <div className="absolute right-0 top-0 w-64 h-64 bg-mare-navy/[0.02] rounded-full blur-3xl -translate-y-1/2 translate-x-1/3"></div>
            <div className="absolute left-0 bottom-0 w-64 h-64 bg-mare-turquoise/[0.05] rounded-full blur-3xl translate-y-1/3 -translate-x-1/3"></div>

            <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-8">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-mare-navy to-mare-navy/80 flex items-center justify-center text-mare-turquoise shrink-0 shadow-lg shadow-mare-navy/20">
                <MapPin strokeWidth={2} className="w-8 h-8" />
              </div>
              
              <div className="flex-1 text-center md:text-left">
                <div className="inline-block px-3 py-1 bg-mare-gold/10 text-mare-gold text-[9px] font-black uppercase tracking-[0.2em] rounded-full mb-3">
                  Nuestra Ubicación
                </div>
                <h3 className="text-2xl font-black text-mare-navy tracking-tight mb-2">
                  {config.delivery.pickupLocations[0].name}
                </h3>
                <p className="text-sm text-gray-500 font-medium mb-4 max-w-md mx-auto md:mx-0">
                  {config.delivery.pickupLocations[0].address}
                </p>
                
                <div className="flex flex-col sm:flex-row items-center sm:items-start justify-center md:justify-start gap-3 sm:gap-6">
              ...
              {config.delivery.pickupLocations[0].mapsUrl && (
                <div className="flex shrink-0 w-full md:w-auto mt-4 md:mt-0">
                  <a 
                    ...
                    className="w-full md:w-auto px-8 py-4 bg-mare-navy text-white font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-black transition-all shadow-xl shadow-mare-navy/20 active:scale-95 flex items-center justify-center gap-2"
                  >
