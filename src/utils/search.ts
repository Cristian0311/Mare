...
export const normalizeText = (text: string): string => {
  return text
    .toLowerCase()
    .normalize('NFD') // Decompose into base character + diacritical marks
...
export const suggestCategories = (query: string) => {
  const normalizedQuery = normalizeText(query);
  if (!normalizedQuery) return [];
  const categories = categoryService.getCategoriesSync();
...
export const suggestSubcategories = (query: string) => {
  const normalizedQuery = normalizeText(query);
  if (!normalizedQuery) return [];
...
export const suggestTags = (query: string) => {
  const normalizedQuery = normalizeText(query);
  if (!normalizedQuery) return [];
